...
if (isset($_GET['id'])){
	...
	<center><a href="?mod=ver_usuario&n=<? echo "$_GET[n]";?>&action=historial_pagos">Volver</a></center><?
	exit;
}
...
$sql = "SELECT * FROM $TABLA_HISTORIAL_PAGOS WHERE $ID_USUARIO=".$id_u;
$res = mysql_query($sql) or die (mysql_error());
...
<table border="2" align="center">
	...
	<tbody><?
		while($fila = mysql_fetch_array($res)){?>
			<tr>
				<td><? echo "$fila[$NUMERO]";?></td>
				<td><? echo "$fila[$FECHA]";?></td>
				<td><? echo "$fila[$CANTIDAD]";?></td>
				<td><a href="?mod=ver_usuario&n=<? echo "$_GET[n]";?>&action=historial_pagos&action2=ver&id=<? echo "$fila[$ID]";?>">< Detalles ></a></td>
			</tr><?
		}?>
	</tbody>
</table>
